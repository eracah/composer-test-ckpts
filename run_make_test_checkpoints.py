...
composer_versions = ['v0.15.1'] # ['v0.13.5', 'v0.14.0', 'v0.14.1', 'v0.15.0',  'v0.16.0']


manual_test_integration = {
    'integration_type': 'git_repo',
    'git_repo': 'eracah/composer-manual-tests',
    'git_branch': 'master',
    'path': '/tmp/composer-manual-tests'
}

fsdp_state_dict_types = ['sharded', 'full', 'local']
...
for fsdp_state_dict_type, precision, sharding_strategy, image, composer_version in itertools.product(fsdp_state_dict_types,
    ...
    cmd = f'pip install pydantic==1.10.12; cd /tmp/composer-manual-tests/test-ckpt && composer -n 2 make_test_ckpt.py {fsdp_state_dict_type} {precision} {sharding_strategy}'
